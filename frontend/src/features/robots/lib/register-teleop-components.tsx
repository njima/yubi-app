...
function GateStatusCard({ ctx }: { ctx: LayoutContext }) {
...
function SubTaskProgressCard({ ctx }: { ctx: LayoutContext }) {
  const { t } = useTranslation();
...
function TaskInformationCard({ ctx }: { ctx: LayoutContext }) {
  const { t } = useTranslation();
...
function SubtaskDetailList({ ctx }: { ctx: LayoutContext }) {
  const { t } = useTranslation();
  const subtasks = [...(ctx.episode?.subtasks ?? [])].sort(
    (a, b) => a.order_index - b.order_index
  );
