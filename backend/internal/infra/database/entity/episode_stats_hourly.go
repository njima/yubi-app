...
type EpisodeStatsHourly struct {
	bun.BaseModel `bun:"table:episode_stats_hourly,alias:esh"`
	Timestamp
	OrgScoped
...
var EpisodeStatsHourlyTableCreator = func(db *bun.DB) *bun.CreateTableQuery {
	return db.NewCreateTable().
		Model((*EpisodeStatsHourly)(nil)).
		IfNotExists().
...
var EpisodeStatsHourlyIdxCreators = []IndexQueryCreator{
	func(db *bun.DB) *bun.CreateIndexQuery {
		return db.NewCreateIndex().
			Model((*EpisodeStatsHourly)(nil)).
			Index("episode_stats_hourly_org_loc_robot_period_idx").
			Unique().
	...
	func(db *bun.DB) *bun.CreateIndexQuery {
		return db.NewCreateIndex().
			Model((*EpisodeStatsHourly)(nil)).
			Index("episode_stats_hourly_period_start_idx").
			Column("period_start")
	},
}

var _ bun.BeforeAppendModelHook = (*EpisodeStatsHourly)(nil)

func (e *EpisodeStatsHourly) BeforeAppendModel(ctx context.Context, query bun.Query) error {
	now := time.Now().UTC()
