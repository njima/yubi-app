...
type EpisodeStatsDaily struct {
	bun.BaseModel `bun:"table:episode_stats_daily,alias:esd"`
	Timestamp
	OrgScoped
...
var EpisodeStatsDailyTableCreator = func(db *bun.DB) *bun.CreateTableQuery {
	return db.NewCreateTable().
		Model((*EpisodeStatsDaily)(nil)).
		IfNotExists().
...
var EpisodeStatsDailyIdxCreators = []IndexQueryCreator{
	func(db *bun.DB) *bun.CreateIndexQuery {
		return db.NewCreateIndex().
			Model((*EpisodeStatsDaily)(nil)).
			Index("episode_stats_daily_org_loc_robot_period_idx").
			Unique().
	...
	func(db *bun.DB) *bun.CreateIndexQuery {
		return db.NewCreateIndex().
			Model((*EpisodeStatsDaily)(nil)).
			Index("episode_stats_daily_period_start_idx").
			Column("period_start")
	},
}

var _ bun.BeforeAppendModelHook = (*EpisodeStatsDaily)(nil)

func (e *EpisodeStatsDaily) BeforeAppendModel(ctx context.Context, query bun.Query) error {
	now := time.Now().UTC()
